# 设计思路
本系统采用STM32F103C8T6作为主控核心，通过OV2640摄像头获取图像数据，利用基于颜色阈值的嵌入式视觉算法实时计算目标像素偏移；该偏移量经比例控制转换为PWM信号，驱动双舵机云台转动，使固联于云台上的HC-SR04超声波传感器对准目标；随后触发40kHz超声波脉冲，捕获回波，结合温度补偿的声速模型实现厘米级精度测距。
